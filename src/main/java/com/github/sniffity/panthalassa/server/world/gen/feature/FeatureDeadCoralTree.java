...
public class FeatureDeadCoralTree extends FeaturePanthalassaDeadCoral {
    public FeatureDeadCoralTree(Codec<NoneFeatureConfiguration> p_i231942_1_) {
        super(p_i231942_1_);
    }
    ...
    protected boolean placeFeature(LevelAccessor p_65490_, Random p_65491_, BlockPos p_65492_, BlockState p_65493_) {
        ...
        if (p_65490_.getBlockState(blockposAdjusted.below()).is(PanthalassaBlocks.PANTHALASSA_ROCK.get())) {
            BlockPos.MutableBlockPos blockpos$mutableblockpos = blockposAdjusted.mutable();
            int i = p_65491_.nextInt(3) + 1;

            for (int j = 0; j < i; ++j) {
                if (!this.placeDeadCoralBlock(p_65490_, p_65491_, blockpos$mutableblockpos, p_65493_)) {
                    return true;
                }
            ...
            for (Direction direction : list.subList(0, k)) {
                ...
                for (int j1 = 0; j1 < l && this.placeDeadCoralBlock(p_65490_, p_65491_, blockpos$mutableblockpos, p_65493_); ++j1) {
                    ++i1;
                    blockpos$mutableblockpos.move(Direction.UP);
