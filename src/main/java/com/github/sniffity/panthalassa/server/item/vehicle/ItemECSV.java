...
public class ItemECSV extends Item {
    ...
    public ItemECSV(Item.Properties p_i48526_2_) {
        super(p_i48526_2_);
    }
    ...
    public InteractionResultHolder<ItemStack> use(Level worldIn, Player player, InteractionHand hand) {
        ...
        if (raytraceresult.getType() == HitResult.Type.MISS) {
            ...
            if (raytraceresult.getType() == HitResult.Type.BLOCK) {
                VehicleECSV vehicleECSV = new VehicleECSV(worldIn, raytraceresult.getLocation().x, raytraceresult.getLocation().y, raytraceresult.getLocation().z);
                vehicleECSV.yRot = player.yRot;
                if (!worldIn.noCollision(vehicleECSV, vehicleECSV.getBoundingBox().inflate(-0.1D))) {
