...
public class FeatureRockSpikes extends Feature<NoneFeatureConfiguration> {
    public FeatureRockSpikes(Codec<NoneFeatureConfiguration> p_66003_) {
        super(p_66003_);
    }
    ...
    public boolean place(FeaturePlaceContext<NoneFeatureConfiguration> p_159882_) {
        ...
        if (!worldgenlevel.getBlockState(pos0).is(PanthalassaBlocks.PANTHALASSA_ROCK.get())) {
            return false;
        } else {
                ...
                for(int i1 = -l; i1 <= l; ++i1) {
                    ...
                    for(int j1 = -l; j1 <= l; ++j1) {
                        ...
                        if ((i1 == 0 && j1 == 0 || !(f1 * f1 + f2 * f2 > f * f)) && (i1 != -l && i1 != l && j1 != -l && j1 != l || !(random.nextFloat() > 0.75F))) {
                            ...
                            if (blockstate.is(PanthalassaBlocks.PANTHALASSA_WATER.get()) || isDirt(blockstate) || blockstate.is(PanthalassaBlocks.PANTHALASSA_ROCK.get())) {
                                this.setBlock(worldgenlevel, pos0.offset(i1, k, j1), PanthalassaBlocks.ANCIENT_ROCK.get().defaultBlockState());
                            }

                            if (k != 0 && l > 1) {
                                blockstate = worldgenlevel.getBlockState(pos0.offset(i1, -k, j1));
                                if (blockstate.is(PanthalassaBlocks.PANTHALASSA_WATER.get()) || blockstate.is(PanthalassaBlocks.PANTHALASSA_ROCK.get())) {
                                    this.setBlock(worldgenlevel, pos0.offset(i1, -k, j1), PanthalassaBlocks.ANCIENT_ROCK.get().defaultBlockState());
                                }
                            }
                        }
            ...
            for(int l1 = -k1; l1 <= k1; ++l1) {
                for(int i2 = -k1; i2 <= k1; ++i2) {
                    ...
                    while(blockpos1.getY() > 50) {
                        ...
                        if (!blockstate1.isAir() && !isDirt(blockstate1) && !blockstate1.is(PanthalassaBlocks.PANTHALASSA_WATER.get()) && !blockstate1.is(PanthalassaBlocks.PANTHALASSA_ROCK.get()) && !blockstate1.is(PanthalassaBlocks.ANCIENT_ROCK.get())) {
                            break;
                        }

                        this.setBlock(worldgenlevel, blockpos1, PanthalassaBlocks.ANCIENT_ROCK.get().defaultBlockState());
