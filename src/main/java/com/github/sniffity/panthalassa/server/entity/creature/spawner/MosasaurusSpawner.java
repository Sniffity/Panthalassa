...
public class MosasaurusSpawner  {

    public MosasaurusSpawner(){
    }
            ...
            if (!(playerentity == null || (playerentity.level.dimension() != PanthalassaDimension.PANTHALASSA))) {
                ...
                if (worldIn.hasChunksAt(blockpos.getX() - 10, blockpos.getY() - 10, blockpos.getZ() - 10, blockpos.getX() + 10, blockpos.getY() + 10, blockpos.getZ() + 10)) {
                    if ((checkValidSpawnArea(worldIn, blockpos))) {
                        if (WorldEntitySpawner.isSpawnPositionOk(EntitySpawnPlacementRegistry.PlacementType.IN_WATER, worldIn, blockpos, PanthalassaEntityTypes.MOSASAURUS.get()));
    ...
    private void spawnInBiome(ServerWorld worldIn, BlockPos blockPos) {
        if (worldIn.getBiome(blockPos).getRegistryName() != null) {
            if (Objects.equals(worldIn.getBiome(blockPos).getRegistryName(), new ResourceLocation(Panthalassa.MODID, "abyssal_overgrowth"))) {
                List<EntityMosasaurus> list = worldIn.getEntitiesOfClass(EntityMosasaurus.class, (new AxisAlignedBB(blockPos)).inflate(32.0D, 32.0D, 32.0D));
                ...
                if (list.isEmpty() && list1.isEmpty()) {
                    this.spawnMosasaurus(blockPos, worldIn);
                }
            }
        }
    }

    private void spawnMosasaurus(BlockPos blockPos, ServerWorld worldIn) {
        EntityMosasaurus mosasaurus = PanthalassaEntityTypes.MOSASAURUS.get().create(worldIn);

        if (mosasaurus != null){
            mosasaurus.moveTo(blockPos, 0.0F, 0.0F);
            if(net.minecraftforge.common.ForgeHooks.canEntitySpawn(mosasaurus, worldIn, blockPos.getX(), blockPos.getY(), blockPos.getZ(), null, SpawnReason.NATURAL) != -1) {
                mosasaurus.finalizeSpawn(worldIn, worldIn.getCurrentDifficultyAt(blockPos), SpawnReason.NATURAL, (ILivingEntityData)null, (CompoundNBT)null);
                worldIn.addFreshEntityWithPassengers(mosasaurus);
            }
        }
