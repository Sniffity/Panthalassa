...
public class PanthalassaVehicle2 extends Entity {
    private static final DataParameter<Integer> TIME_SINCE_HIT = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.VARINT);
    private static final DataParameter<Integer> FORWARD_DIRECTION = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.VARINT);
    private static final DataParameter<Float> DAMAGE_TAKEN = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.FLOAT);
    private static final DataParameter<Integer> BOAT_TYPE = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.VARINT);
    private static final DataParameter<Boolean> LEFT_PADDLE = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Boolean> RIGHT_PADDLE = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.BOOLEAN);
    private static final DataParameter<Integer> ROCKING_TICKS = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.VARINT);

    //Attributes
    private static final DataParameter<Float> MAX_HEALTH = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.FLOAT);
    private static final DataParameter<Float> HEALTH = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.FLOAT);
    private static final DataParameter<Float> ARMOR = EntityDataManager.createKey(PanthalassaVehicle2.class, DataSerializers.FLOAT);
    ...
    private PanthalassaVehicle2.Status previousStatus;
    ...
    public PanthalassaVehicle2(EntityType<?> type, World world) {
        super(type, world);
        this.preventEntitySpawning = true;
    }

    public PanthalassaVehicle2(World worldIn, double x, double y, double z) {
    ...
    protected void registerData() {
        ...
        this.dataManager.register(BOAT_TYPE, PanthalassaVehicle2.Type.OAK.ordinal());
        ...
        this.dataManager.register(ARMOR, 10F);
    }
    ...
    public boolean attackEntityFrom(DamageSource source, float amount) {
        if (this.isInvulnerableTo(source)) {
            ...
            //Alter forward direction on hit
            this.setForwardDirection(-this.getForwardDirection());
            if (this.getHealth() < 0.0F) {
                this.vehicleDestroy((LivingEntity) source.getTrueSource());
                this.remove();
    ...
    public void applyEntityCollision(Entity entityIn) {
        if (entityIn instanceof PanthalassaVehicle2) {
    ...
    //Each tick, verify if it's on water or on air.
    public void tick() {
        //Update status
        this.previousStatus = this.status;
        //Get its current status, if it's on aiar...
        this.status = this.getBoatStatus();
        if (this.status != PanthalassaVehicle2.Status.UNDER_WATER && this.status != PanthalassaVehicle2.Status.UNDER_FLOWING_WATER) {
     ...
     */
    private PanthalassaVehicle2.Status getBoatStatus() {
        PanthalassaVehicle2.Status panthalassavehicle2$status = this.getUnderwaterStatus();
        //Item?
        if (panthalassavehicle2$status != null) {
            this.waterLevel = this.getBoundingBox().maxY;
            return panthalassavehicle2$status;
        } else if (this.checkInWater()) {
            return PanthalassaVehicle2.Status.IN_WATER;
        } else {
            float f = this.getVehicleGlide();
            if (f > 0.0F) {
                this.boatGlide = f;
                return PanthalassaVehicle2.Status.ON_LAND;
            } else {
                return PanthalassaVehicle2.Status.IN_AIR;
            }
        }
    }


    //Check for waterLevel above the vehicle.
     ...
     */
    @Nullable
    private PanthalassaVehicle2.Status getUnderwaterStatus() {
        AxisAlignedBB axisalignedbb = this.getBoundingBox();
                ...
                for (int i2 = i1; i2 < j1; ++i2) {
                    ...
                    if (fluidstate.isTagged(FluidTags.WATER) && d0 < (double) ((float) blockpos$mutable.getY() + fluidstate.getActualHeight(this.world, blockpos$mutable))) {
                        if (!fluidstate.isSource()) {
                            return PanthalassaVehicle2.Status.UNDER_FLOWING_WATER;
                        }
        ...
        return flag ? PanthalassaVehicle2.Status.UNDER_WATER : null;
    }
    ...
    private void updateMotion() {
        ...
        //d2 is used to set the vehicle's Y motion. If d2 is greather thn 0, The new motion will be current motion (Y) + d2*scaling factor
        //On X and Y it will retain its motion.
        //d2 is an override of sorts.
        double d2 = 0.0D;
        this.momentum = 0.05F;
        //IF it used to be on the air, and now it's no longer in the air, nor is it in land... Meaning, if it lands in water....
        //Implement behavior similar to Wyrmroost's leap on BFLs.
        if (this.previousStatus == PanthalassaVehicle2.Status.IN_AIR && this.status != PanthalassaVehicle2.Status.IN_AIR && this.status != PanthalassaVehicle2.Status.ON_LAND) {
            ...
            this.lastYd = 0.0D;
            this.status = PanthalassaVehicle2.Status.IN_WATER;
            //If it's in water....
        } else {
            if (this.status == PanthalassaVehicle2.Status.IN_WATER) {
                //Check its height in water.
                d2 = (this.waterLevel - this.getPosY()) / (double) this.getHeight();
                //Add momentum, it can move.
                this.momentum = 0.9F;
            } else if (this.status == PanthalassaVehicle2.Status.UNDER_FLOWING_WATER) {
                //Move downwards if underflowing water?
                d1 = -7.0E-4D;
                //Add momentum, it can move.
                this.momentum = 0.9F;
            } else if (this.status == PanthalassaVehicle2.Status.UNDER_WATER) {
                d2 = (double) 0.01F;
                this.momentum = 0.45F;
            } else if (this.status == PanthalassaVehicle2.Status.IN_AIR) {
                this.momentum = 0.9F;
            } else if (this.status == PanthalassaVehicle2.Status.ON_LAND) {
    ...
    protected void readAdditional(CompoundNBT compound) {
        if (compound.contains("Type", 8)) {
            this.setBoatType(PanthalassaVehicle2.Type.getTypeFromString(compound.getString("Type")));
        }
        ...
        if (!this.isPassenger()) {
            if (onGroundIn) {
                if (this.fallDistance > 3.0F) {
                    if (this.status != PanthalassaVehicle2.Status.ON_LAND) {
    ...
    public void setBoatType(PanthalassaVehicle2.Type boatType) {
        this.dataManager.set(BOAT_TYPE, boatType.ordinal());
    }

    public PanthalassaVehicle2.Type getBoatType() {
        return PanthalassaVehicle2.Type.byId(this.dataManager.get(BOAT_TYPE));
    }
    ...
    public boolean canSwim() {
        return this.status == PanthalassaVehicle2.Status.UNDER_WATER || this.status == PanthalassaVehicle2.Status.UNDER_FLOWING_WATER;
    }
    ...
    public static enum Type {
         ...
         */
        public static PanthalassaVehicle2.Type byId(int id) {
            PanthalassaVehicle2.Type[] aboatentity$type = values();
        ...
        public static PanthalassaVehicle2.Type getTypeFromString(String nameIn) {
            PanthalassaVehicle2.Type[] aboatentity$type = values();
    ...
    {
        this.dataManager.set(ARMOR, Math.min(this.getMaxHealth(), armor));
    }
